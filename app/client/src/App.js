...
export default function App() {
  ...
  useEffect(() => {
    async function fetchConfigs() {
      try {
        // Admin config to store in context
        const adminConfigResult = await ConfigService.getAdminConfig();
        setConfig(adminConfigResult.data);

        // UI config to store in settings
        const uiConfigResult = await ConfigService.getConfig();
        setSetting('ui_config', uiConfigResult.data);
      } catch (err) {
  ...
  const drawerOpen =
    getSetting('drawerOpen') === undefined ? true : getSetting('drawerOpen');
